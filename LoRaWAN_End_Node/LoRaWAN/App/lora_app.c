...
/* External variables ---------------------------------------------------------*/
/* USER CODE BEGIN EV */
volatile int muon1 = 0;
volatile float voltage_array[100][5] = {0};
int Sensor_read_data = 1;
__IO   uint16_t   aADCxConvertedData[5];
...
void HAL_COMP_TriggerCallback(COMP_HandleTypeDef *hcomp)
{
  ...
  if (COMP1OutputLevel>=COMP_OUTPUT_LEVEL_HIGH)
	  ...
	  //TIMEARRAY[muon1] = UTIL_TIMER_GetCurrentTime();
	  muon1++;
	  /*if (ubDmaTransferStatus == 1)
	  {
		  while(q<5)
		  {
		  	  voltage_array[z][q]=(uint32_t)aADCxConvertedData[q] * 3.3 / (4095);
		  	  q++;
		  }
		  z++;
  ...
  if (LmHandlerIsBusy() == false)
  {
	if(Sensor_read_data)
	{
	   ...
	   //APP_LOG(TS_ON, VLEVEL_M, "VDDA: %d\r\n", batteryLevel);
		...
		// muon data sent as analog output
		CayenneLppAddAnalogOutput(channel++, (uint16_t)(muon1));
		/*while (i<100)
		{
		CayenneLppAddAnalogOutput(channel++, (uint16_t)(TIMEARRAY[i]));
		i++;
		if (TIMEARRAY[i]==0)
		{
			break;
		...
		while (p<5)
		{
		CayenneLppAddAnalogOutput(channel++, (uint16_t)voltage_array[0][p]);
		p++;
		}
		//y++;*/
		/*if (voltage_array[y]==0)
		{
			break;
		}*/
		//}
